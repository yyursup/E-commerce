...
import { getAccessToken, getRefreshToken, clearAccessToken } from './auth'
...
// Add a response interceptor for global error handling and 401 refresh/redirect
      ...
      if (!isAuthEndpoint && originalRequest && !originalRequest._retry) {
        ...
        if (refreshToken) {
          ...
          try {
            ...
            console.warn('Auto refresh token failed in api instance:', refreshErr);
          }
        }
      }

      // If refresh failed or not available, clean logout and redirect
      if (!isAuthEndpoint) {
        useAuthStore.getState().logout();
        clearAccessToken();

        const currentPath = window.location.pathname;
        const isAuthPage = currentPath.includes('/login') || currentPath.includes('/register') || currentPath.includes('/verify') || currentPath.includes('/forgot-password');
