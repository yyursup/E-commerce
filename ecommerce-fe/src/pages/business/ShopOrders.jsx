...
import { ORDER_STATUSES } from './components/order/orderHelpers'
...
export default function ShopOrders() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const ordersData = await orderService.getShopOrders(status)
    ...
    <div>
      ...
      {!loading && !error && orders.length > 0 && (
        <div className="space-y-4">
          {orders.map((order) => (
            <ShopOrderCard key={order.id} order={order} isDark={isDark} />
          ))}
        </div>
      )}
    </div>
