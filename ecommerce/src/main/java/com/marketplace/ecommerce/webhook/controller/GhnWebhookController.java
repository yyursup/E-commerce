 ...
 * Khi Type=Switch_status và Status=delivered (hoặc tương đương), set order
 * DELIVERED.
 * <p>
 * Luôn trả HTTP 200 theo yêu cầu GHN (nếu khác 200, GHN bắn lại 10 lần, mỗi lần
 * cách 5 giây).
...
public class GhnWebhookController {
    ...
    public ResponseEntity<Void> handleGhnWebhook(@RequestBody GhnWebhookPayload payload) {
        ...
        String clientOrderCode = payload.getClientOrderCode() != null ? payload.getClientOrderCode()
                : payload.getPartnerId();

        boolean isDelivered = WebhookConstant.STATUS_DELIVERED_ID == (payload.getDeliveryStatusId() != null
                ? payload.getDeliveryStatusId()
                : -1)
        ...
        if (isDelivered && (orderCode != null || clientOrderCode != null)) {
            try {
                ...
                if (updated) {
                    log.info("GHN webhook: đã set DELIVERED (OrderCode={}, ClientOrderCode={})", orderCode,
                            clientOrderCode);
                } else {
                    log.debug("GHN webhook: không tìm thấy đơn SHIPPED (OrderCode={}, ClientOrderCode={})", orderCode,
                            clientOrderCode);
                }
            } catch (Exception e) {
                log.warn("GHN webhook: lỗi khi xử lý delivered – OrderCode={}, ClientOrderCode={}: {}", orderCode,
                        clientOrderCode, e.getMessage());
            }
        }
        return ResponseEntity.ok().build();
    }
