...
class RequestServiceImplTest {
    ...
    void approveSellerRegistration_Success() {
        ...
        when(shopService.createShop(eq(ownerUser), eq("ABC Tech Store"), eq(mockRequest), eq(mockSeller))).thenReturn(createdShop);
        when(roleRepository.findByRoleName("BUSINESS")).thenReturn(Optional.of(businessRole));

        // When
        RequestResponse response = requestService.approveSellerRegistration(requestId, adminAccountId, "Đã duyệt đơn đăng ký");
    ...
    void getDetails_SellerRegistration_Success() {
        ...
        assertTrue(details.getDetail() instanceof RegisterSellerResponse);
